...
class MTL_LMDBDataset_test(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            char_key = 'char-%09d' % index
            style_key = 'style-%09d' % index
            dynasty_key = 'dynasty-%09d' % index
            ...
            char = str(txn.get(char_key.encode()).decode())
            style = str(txn.get(style_key.encode()).decode())
            dynasty = str(txn.get(dynasty_key.encode()).decode())
        ...
        char = int(char)
        dynasty = int(dynasty)
        return (img, label, char, style, dynasty)



class MTL_LMDBDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        img, label, style = zip(*batch)
        img = np.stack([i for i in img], axis=0)
        label = np.stack([i for i in label], axis=0)
        char = np.stack([i for i in char], axis=0)
        style = np.stack([i for i in style], axis=0)
        dynasty = np.stack([i for i in dynasty], axis=0)
